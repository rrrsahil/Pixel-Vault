    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/upload', formData, {
        headers: {
          ...
          'Authorization': `Bearer ${localStorage.getItem('token') || ''}` // Add JWT if it exists
        }
      });
      ...
      if (response.data.isDuplicate) {
          toast.info('Exact duplicate image detected; providing the pre-existing URL to save space.');
      } else {
      ...
      toast.error(error.response?.data?.message || 'Error uploading image. Please check limits.');
    } finally {
      setUploading(false);
    }
  };
          ...
          <div className="preview-container">
            <div className="preview-card">
              <div className="preview-image-wrapper">
                 <img src={preview || uploadedUrl} alt="Preview" className="preview-image" />
                 {uploading && (
